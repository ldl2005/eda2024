...
struct polygon{
    vector<line>lines;
    int x_max=-1e9,y_max=-1e9,x_min=1e9,y_min=1e9;//相当于marker
};
//     layer
//       |
...
pattern maker_get_pattern_from_template(point marker[]){
    line marker_line[4];
    pattern res_pattern;res_pattern.layers.clear();
    for(int i=0;i<4;i++){
        if(i==3){
            marker_line[3]={marker[3],marker[0]};
        }
        marker_line[0]={marker[i],marker[i+1]};
    }
    for(int i=0;i<(int)template_file.layers.size();i++){
        ...
        for(int j=0;j<(int)tmp_layer.size();j++){//处理图形
            ...
            for(int cnt_line=0;i<(int)tmp_polygon.lines.size();i++){
                ...
                if(cnt_line==0){
                    points.push_back(tmp_polygon_line.first);
                    points.push_back(tmp_polygon_line.second);
                }
                else if(cnt_line==tmp_polygon.lines.size()-1){
                    points.push_back(tmp_polygon_line.first);
                }
            }
            ...
            for(int cnt_points=0;cnt_points<points.size();cnt_points++){
            ...
            //全部在marker内部
            if(cnt_point_in_marker==cnt_points_of_tmp_polygon)tmp_layer.push_back(tmp_polygon);
            //全部不在
            if(cnt_point_in_marker==0)continue;
            //部分在//重新处理
...
int main(){
    //输入部分
    read_template_file("C:/Users/29415/Desktop/eda2024/public/small/small_template.txt");
    ...
    for(int i=0;i<patterns.size();i++){//每一个模板
        int id=i+1;//模板的id
